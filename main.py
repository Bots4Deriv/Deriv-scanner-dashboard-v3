...
ws_connection = None  # Track WebSocket for trading
...
async def get_balance():
    """Get balance using existing WebSocket connection"""
    if not ws_connection or not DERIV_API_TOKEN:
        return 0.0
    try:
        await ws_connection.send(json.dumps({"authorize": DERIV_API_TOKEN}))
        response = await asyncio.wait_for(ws_connection.recv(), timeout=5.0)
        data = json.loads(response)
        
        if "authorize" in data:
            await ws_connection.send(json.dumps({"balance": 1, "subscribe": 0}))
            response = await asyncio.wait_for(ws_connection.recv(), timeout=5.0)
            data = json.loads(response)
    ...
    return bot_status.get("balance", 0.0)

async def place_trade(contract_type="CALL"):
    """Place trade using existing WebSocket"""
    ...
    try:
        # Ensure authorized
        await ws_connection.send(json.dumps({"authorize": DERIV_API_TOKEN}))
        await asyncio.wait_for(ws_connection.recv(), timeout=3.0)
        ...
        await ws_connection.send(json.dumps(contract))
        ...
        if "buy" in data:
            ...
            asyncio.create_task(monitor_trade(trade_id, contract_id))
            
            msg = (f"🚀 TRADE: {contract_type} ${stake:.2f}\n"
                   f"ID: {trade_id} | P&L: ${bot_status['daily_pnl']:.2f}")
            logger.info(msg)
            send_telegram(msg)
            return trade_info
...
async def monitor_trade(trade_id, contract_id):
    """Monitor trade via existing WebSocket"""
    try:
        subscribe_msg = {
        ...
        while not shutdown_event.is_set():
            try:
                msg = await asyncio.wait_for(ws_connection.recv(), timeout=30.0)
                data = json.loads(msg)
                ...
                if "proposal_open_contract" in data:
                    ...
                    if contract.get("is_sold"):
                        ...
                        msg = (f"{emoji} {status} ${profit:+.2f}\n"
                               f"ID: {trade_id}\n"
                               f"Daily: ${bot_status['daily_pnl']:.2f}")
                        logger.info(msg)
                        send_telegram(msg)
...
async def stream_ticks():
    global prices, times, price_history, previous_price, general_trend_history, ws_connection
    url = f"wss://ws.derivws.com/websockets/v3?app_id={DERIV_APP_ID}"
    
    bot_status["running"] = True
    bot_status["started_at"] = get_eat_timestamp()
    
    logger.info(f"🚀 Bot starting | Symbol: {SYMBOL} | Trading: {AUTO_TRADE_ENABLED}")

    while not shutdown_event.is_set():
        try:
            async with websockets.connect(url, ping_interval=20, ping_timeout=20) as ws:
                ws_connection = ws
                logger.info(f"✅ Connected to Deriv")
                
                if AUTO_TRADE_ENABLED:
                    await get_balance()
                    logger.info(f"💵 Balance: ${bot_status['balance']:.2f}")
...
async def start_server():
    app = web.Application()
    app.router.add_get('/health', health_check)  # Railway health check
...
async def main():
    ...
    # Railway requires immediate server start
...
if __name__ == "__main__":
    ...
    if AUTO_TRADE_ENABLED and not DERIV_API_TOKEN:
        logger.warning("⚠️ AUTO_TRADE_ENABLED but no DERIV_API_TOKEN - trading disabled")
